...
app.route('/cityName').get(async function (req, res) {
    let maxRows = 8;
    return await axios({
        method: 'GET',
        url: `http://api.geonames.org/findNearbyPlaceNameJSON?lat=${req.query.lat}&lng=${req.query.lng}&username=carraragiovanni`
    }).then(function (response) {
        res.send({
